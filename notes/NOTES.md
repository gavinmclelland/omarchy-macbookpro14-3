...
6. **Thin/hollow/quiet, no boom:** two volume stages (filter 40%/−24 dB **and** PCM/4ch −8 dB). Hardware 4ch locked at 0 dB.
...
**Layout 57 DSP is live** (not a new kernel driver). Keep davidjo. Chained `bq_*`. Tones vs 500 Hz at 1 kHz: invert off −10 dB; invert on −4 dB; invert + **5-sample woofer delay −0.7 dB**. Keep invert + delay. Lodi Dodi’s deeper 1 kHz dip on the mic was the track + Apple −20 dB @ 280 Hz. Omarchy volume keys resolve through `cs8409_speakers` to analog-surround-40 (`omarchy-audio-output-sink`). Locking that physical node froze the Touch Bar. DSP sink stays at full scale (`cs8409-dsp-unity.service`); TB/OSD move 4ch/PCM. One player when listening. Parked LR4: `pipewire/60-cs8409-lr4.conf`. [#14](https://github.com/gavinmclelland/omarchy-macbookpro14-3/issues/14).
...
Idle: live `idle_timeout=300` `dim_timeout=150` (lock / screensaver). Not sat idle to confirm dim. keyd grab may mean only TB/trackpad reset `last_event_time`. [#1](https://github.com/gavinmclelland/omarchy-macbookpro14-3/issues/1).
...
| Issue | Needs |
| --- | --- |
| [#1](https://github.com/gavinmclelland/omarchy-macbookpro14-3/issues/1) TB dim | Live `idle=300 dim=150`. Sit idle ~150 s (no trackpad/TB); keyd grab may starve keyboard wake. |
...
| [#14](https://github.com/gavinmclelland/omarchy-macbookpro14-3/issues/14) speaker quality | Host live. Best test: Still D.R.E. Cabinet eats 80–160 Hz, adds 1–3 kHz. BuzzKill/thermal still out. PR #18 |
