...
6. **Thin/hollow/quiet, no boom:** two volume stages (filter 40%/−24 dB **and** PCM/4ch −8 dB). Woofer invert cancelled bass in the overlap. Now: hardware 4ch locked at 0 dB, LR4 @ 1000 Hz, no invert, woofer shelf +8 dB @ 100 Hz, filter sink ~70%. ACP chmap advertised `FL,FR,LFE,LFE`; `channelmix.disable` so RL/RR are not remixed away.
